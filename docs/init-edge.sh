...
cat <<EOF > kustomize/fl-edge/config/spire-server-creds.env
...
cat <<EOF > kustomize/fl-edge/config/fl-operator-params.env
fl_orchestrator_sni=$FL_ORCHESTRATOR_SNI
fl_orchestrator_port=8443
EOF

# Deploy kind cluster that uses fl-studio's container registry
